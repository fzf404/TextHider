...
const { pipe, intersection, indexOf, curry, __, slice, split, join, map } = require('ramda')
...
const zwcOperations = (zwc) => {
  ...
  const detach = (str) => {
    const eachWords = [str[0], str.slice(1)]
...
const embed = (cover, secret) => {
  const arr = [cover[0], cover.slice(1)]
